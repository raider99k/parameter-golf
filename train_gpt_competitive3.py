"""
Competitive3: a dense competitive line built on the stable competitive2 harness.

The goal is selective borrowing:
- keep competitive2's clean training / eval / export path
- drop the weak BitLinear + fast/meta-first core
- add the common proven architecture pieces from the strongest leaderboard lines
"""
...
class Hyperparameters:
    ...
    _unique_default = os.environ.get("NUM_UNIQUE_ENGINES", "11")
    ...
    train_meta_every = int(os.environ.get("TRAIN_META_EVERY", 0))
    ...
    train_meta_steps = int(os.environ.get("TRAIN_META_STEPS", 0))
    ...
    def __init__(self):
        ...
        self.warmdown_iters = int(os.environ.get("WARMDOWN_ITERS", 3500))
        self.warmup_steps = int(os.environ.get("WARMUP_STEPS", 20))
        self.train_batch_tokens = int(os.environ.get("TRAIN_BATCH_TOKENS", 786_432))
        ...
        unique_default = os.environ.get("NUM_UNIQUE_ENGINES", str(self.num_layers))
        self.num_heads = int(os.environ.get("NUM_HEADS", 8))
        self.mlp_mult = float(os.environ.get("MLP_MULT", 3.0))
        ...
        self.xsa_last_n = int(os.environ.get("XSA_LAST_N", 4))
        self.rope_dims = int(os.environ.get("ROPE_DIMS", 16))
        self.ln_scale = bool(int(os.environ.get("LN_SCALE", "1")))
        self.dtg_enabled = bool(int(os.environ.get("DTG_ENABLED", "0")))
        self.ve_enabled = bool(int(os.environ.get("VE_ENABLED", "1")))
        self.ve_dim = int(os.environ.get("VE_DIM", 128))
        self.ve_layers = os.environ.get("VE_LAYERS", "9,10")

        # Optimizer hyperparameters.
        self.embed_lr = float(os.environ.get("EMBED_LR", 0.6))
        self.head_lr = float(os.environ.get("HEAD_LR", 0.008))
        self.tied_embed_lr = float(os.environ.get("TIED_EMBED_LR", 0.035))
        self.tied_embed_init_std = float(os.environ.get("TIED_EMBED_INIT_STD", 0.005))
        self.matrix_lr = float(os.environ.get("MATRIX_LR", 0.025))
        self.scalar_lr = float(os.environ.get("SCALAR_LR", 0.025))
        ...
        self.train_meta_every = int(os.environ.get("TRAIN_META_EVERY", 0))
        ...
        self.train_meta_steps = int(os.environ.get("TRAIN_META_STEPS", 0))
...
class Rotary(nn.Module):
    # Compile-safe RoPE cache with optional partial-RoPE dimensions and long-context scaling.
    ...
    def _build_cache(self, seq_len: int, device: torch.device) -> tuple[Tensor, Tensor]:
        rd = self.rope_dims
        if seq_len > self.train_seq_len and rd > 2:
    ...
    def forward(self, seq_len: int, device: torch.device, dtype: torch.dtype) -> tuple[Tensor, Tensor]:
        if torch_is_compiling():
            cos, sin = self._build_cache(seq_len, device)
...
class CausalSelfAttention(nn.Module):
    ...
    def _xsa_efficient(self, y: Tensor, v: Tensor) -> Tensor:
        y_t = y.transpose(1, 2)
        v_t = v.transpose(1, 2)
        bsz, seqlen, num_heads, dim = y_t.shape
        num_kv_heads = v_t.size(-2)
        group = num_heads // num_kv_heads
        y_grouped = y_t.reshape(bsz, seqlen, num_kv_heads, group, dim)
        v_norm = F.normalize(v_t, dim=-1).unsqueeze(-2)
        proj = (y_grouped * v_norm).sum(dim=-1, keepdim=True) * v_norm
        return (y_grouped - proj).reshape(bsz, seqlen, num_heads, dim).transpose(1, 2)
...
class Block(nn.Module):
    def __init__(
        ...
        self.attn = attn if attn is not None else CausalSelfAttention(
            dim, num_heads, num_kv_heads, rope_base, qk_gain_init, train_seq_len, rope_dims=rope_dims
        )
    ...
    def forward(self, x: Tensor, x0: Tensor, fast_state: dict[str, Tensor] | None = None, v_embed: Tensor | None = None) -> Tensor:
        del fast_state
...
class GPT(nn.Module):
    def __init__(
        ...
        super().__init__()
        del fast_rank, fast_gate_init
        if use_fast_adapters:
            raise ValueError("competitive3 disables fast adapters; use USE_FAST_ADAPTERS=0")
        ...
        self.ve_enabled = bool(int(os.environ.get("VE_ENABLED", "1")))
        self.ve_dim = int(os.environ.get("VE_DIM", 128))
        self.ve_layers_raw = os.environ.get("VE_LAYERS", "9,10")
        ...
        self.bigram = (
            BigramHashEmbedding(self.bigram_vocab_size, self.bigram_dim, model_dim)
            if self.bigram_vocab_size > 0 else None
        )
        ...
        self.attn_bank = nn.ModuleList([
            ...
            for _ in range(num_unique_attn)
        ])
        self.mlp_bank = nn.ModuleList([MLP(model_dim, mlp_mult) for _ in range(num_unique_mlp)])
        ...
        if self.xsa_last_n > 0:
            for i in range(max(0, num_layers - self.xsa_last_n), num_layers):
                self.blocks[i].attn.use_xsa = True

        self._ve_target_dim = num_kv_heads * (model_dim // num_heads)
        self.ve_layer_indices = [int(x) for x in self.ve_layers_raw.split(",") if x.strip()] if self.ve_enabled else []
        if self.ve_layer_indices:
            self.ve_shared = ValueEmbedding(vocab_size, self.ve_dim, self._ve_target_dim)
    ...
    def _get_ve(self, layer_idx: int, input_ids: Tensor, ve_cache: dict[str, Tensor] | None = None) -> Tensor | None:
...
def main(argv: list[str] | None = None) -> None:
    ...
    log0(
        f"competitive3_core:dense bigram_vocab:{args.bigram_vocab_size} bigram_dim:{args.bigram_dim} "
        f"xsa_last_n:{args.xsa_last_n} active_xsa_layers:{xsa_layers}"
    )
    log0(
        f"value_embedding:enabled={args.ve_enabled} ve_dim:{args.ve_dim} ve_layers:{args.ve_layers} "
        f"partial_sharing attn:{args.num_unique_attn}/{args.num_layers} mlp:{args.num_unique_mlp}/{args.num_layers}"
    )
    log0(
    ...
    log0(
        f"dense_core:True use_factor_embed:{args.use_factor_embed} rope_dims:{args.rope_dims} "
        f"ln_scale:{args.ln_scale} dtg_enabled:{args.dtg_enabled}"
    )
