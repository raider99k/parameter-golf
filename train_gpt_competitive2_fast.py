...
class Hyperparameters:
    ...
    # Training-speed approximations
    sparse_mlp_24 = bool(int(os.environ.get("SPARSE_MLP_24", "1")))
    sparse_mlp_start = int(os.environ.get("SPARSE_MLP_START", "0"))
    sparse_mlp_last_n = int(os.environ.get("SPARSE_MLP_LAST_N", "4"))
    attn_window_start = int(os.environ.get("ATTN_WINDOW_START", "256"))
    attn_window_final = int(os.environ.get("ATTN_WINDOW_FINAL", "0"))
    attn_window_warmup_steps = int(os.environ.get("ATTN_WINDOW_WARMUP_STEPS", "1000"))
    progressive_depth_start = int(os.environ.get("PROGRESSIVE_DEPTH_START", "6"))
    progressive_depth_warmup_steps = int(os.environ.get("PROGRESSIVE_DEPTH_WARMUP_STEPS", "1000"))
    selective_mlp_tokens = bool(int(os.environ.get("SELECTIVE_MLP_TOKENS", "1")))
    selective_mlp_keep_start = float(os.environ.get("SELECTIVE_MLP_KEEP_START", "0.5"))
    selective_mlp_keep_end = float(os.environ.get("SELECTIVE_MLP_KEEP_END", "1.0"))
    selective_mlp_warmup_steps = int(os.environ.get("SELECTIVE_MLP_WARMUP_STEPS", "1200"))
    speed_schedule_update_every = int(os.environ.get("SPEED_SCHEDULE_UPDATE_EVERY", "50"))
    ...
    def __init__(self):
        ...
        self.compile_mode = os.environ.get("COMPILE_MODE", "max-autotune-no-cudagraphs")

        # Training-speed approximations
        self.sparse_mlp_24 = bool(int(os.environ.get("SPARSE_MLP_24", "1")))
        self.sparse_mlp_start = int(os.environ.get("SPARSE_MLP_START", "0"))
        self.sparse_mlp_last_n = int(os.environ.get("SPARSE_MLP_LAST_N", "4"))
        self.attn_window_start = int(os.environ.get("ATTN_WINDOW_START", "256"))
        self.attn_window_final = int(os.environ.get("ATTN_WINDOW_FINAL", "0"))
        self.attn_window_warmup_steps = int(os.environ.get("ATTN_WINDOW_WARMUP_STEPS", "1000"))
        self.progressive_depth_start = int(os.environ.get("PROGRESSIVE_DEPTH_START", "6"))
        self.progressive_depth_warmup_steps = int(os.environ.get("PROGRESSIVE_DEPTH_WARMUP_STEPS", "1000"))
        self.selective_mlp_tokens = bool(int(os.environ.get("SELECTIVE_MLP_TOKENS", "1")))
        self.selective_mlp_keep_start = float(os.environ.get("SELECTIVE_MLP_KEEP_START", "0.5"))
        self.selective_mlp_keep_end = float(os.environ.get("SELECTIVE_MLP_KEEP_END", "1.0"))
        self.selective_mlp_warmup_steps = int(os.environ.get("SELECTIVE_MLP_WARMUP_STEPS", "1200"))
        self.speed_schedule_update_every = int(os.environ.get("SPEED_SCHEDULE_UPDATE_EVERY", "50"))
...
CLI_OVERRIDE_ENV_KEYS = frozenset({
    ...
    "ATTN_WINDOW_START",
    "ATTN_WINDOW_FINAL",
    "ATTN_WINDOW_WARMUP_STEPS",
    "PROGRESSIVE_DEPTH_START",
    "PROGRESSIVE_DEPTH_WARMUP_STEPS",
    "SELECTIVE_MLP_TOKENS",
    "SELECTIVE_MLP_KEEP_START",
    "SELECTIVE_MLP_KEEP_END",
    "SELECTIVE_MLP_WARMUP_STEPS",
    "SPEED_SCHEDULE_UPDATE_EVERY",
    "USE_FACTOR_EMBED",
    "USE_FAST_ADAPTERS",
    "USE_PROJECTION_QAT",
...
class SpeedRuntimeState:
    def __init__(self):
        self.step = 0
        self.total_layers = 0
        self.attn_window: int | None = None
        self.active_layers: int | None = None
        self.selective_keep = 1.0
        self.selective_last_n = 0
        self.sparse_mlp_enabled = False


SPEED_RT = SpeedRuntimeState()


def snapshot_speed_runtime() -> dict[str, object]:
    return {
        "step": SPEED_RT.step,
        "total_layers": SPEED_RT.total_layers,
        "attn_window": SPEED_RT.attn_window,
        "active_layers": SPEED_RT.active_layers,
        "selective_keep": SPEED_RT.selective_keep,
        "selective_last_n": SPEED_RT.selective_last_n,
        "sparse_mlp_enabled": SPEED_RT.sparse_mlp_enabled,
    }


def restore_speed_runtime(snapshot: dict[str, object]) -> None:
    SPEED_RT.step = int(snapshot["step"])
    SPEED_RT.total_layers = int(snapshot["total_layers"])
    SPEED_RT.attn_window = snapshot["attn_window"]
    SPEED_RT.active_layers = snapshot["active_layers"]
    SPEED_RT.selective_keep = float(snapshot["selective_keep"])
    SPEED_RT.selective_last_n = int(snapshot["selective_last_n"])
    SPEED_RT.sparse_mlp_enabled = bool(snapshot["sparse_mlp_enabled"])


def reset_speed_runtime(total_layers: int) -> None:
    SPEED_RT.total_layers = max(int(total_layers), 0)
    SPEED_RT.attn_window = None
    SPEED_RT.active_layers = SPEED_RT.total_layers
    SPEED_RT.selective_keep = 1.0
    SPEED_RT.selective_last_n = 0
    SPEED_RT.sparse_mlp_enabled = False


def _runtime_frac(step: int, warmup_steps: int) -> float:
    if warmup_steps <= 0:
        return 1.0
    return min(float(step) / float(warmup_steps), 1.0)


def update_speed_runtime(args, total_layers: int, step: int) -> None:
    SPEED_RT.step = int(step)
    SPEED_RT.total_layers = max(int(total_layers), 0)
    schedule_bucket = max(int(args.speed_schedule_update_every), 1)
    scheduled_step = (int(step) // schedule_bucket) * schedule_bucket

    if args.attn_window_start > 0:
        attn_frac = _runtime_frac(scheduled_step, args.attn_window_warmup_steps)
        final_window = args.train_seq_len if args.attn_window_final <= 0 else min(args.attn_window_final, args.train_seq_len)
        start_window = min(args.attn_window_start, final_window)
        attn_window = max(1, int(round(start_window + attn_frac * (final_window - start_window))))
        SPEED_RT.attn_window = None if attn_window >= args.train_seq_len else attn_window
    else:
        SPEED_RT.attn_window = None

    depth_start = max(1, min(args.progressive_depth_start, SPEED_RT.total_layers))
    depth_frac = _runtime_frac(scheduled_step, args.progressive_depth_warmup_steps)
    SPEED_RT.active_layers = max(
        1,
        min(SPEED_RT.total_layers, int(round(depth_start + depth_frac * (SPEED_RT.total_layers - depth_start)))),
    )

    if args.selective_mlp_tokens:
        keep_frac = max(
            0.05,
            min(
                1.0,
                args.selective_mlp_keep_start
                + _runtime_frac(scheduled_step, args.selective_mlp_warmup_steps)
                * (args.selective_mlp_keep_end - args.selective_mlp_keep_start),
            ),
        )
        SPEED_RT.selective_keep = keep_frac
        SPEED_RT.selective_last_n = max(0, int(args.sparse_mlp_last_n))
    else:
        SPEED_RT.selective_keep = 1.0
        SPEED_RT.selective_last_n = 0

    SPEED_RT.sparse_mlp_enabled = bool(args.sparse_mlp_24 and scheduled_step >= args.sparse_mlp_start)


def _top2_of_4(x: Tensor) -> Tensor:
    if x.size(-1) < 4 or (x.size(-1) % 4) != 0:
        return x
    xg = x.view(*x.shape[:-1], -1, 4)
    idx = xg.topk(k=2, dim=-1, sorted=False).indices
    mask = torch.zeros_like(xg)
    mask.scatter_(-1, idx, 1.0)
    return (xg * mask).view_as(x)
    ...
    def forward(self, x: Tensor, fast_state: dict[str, Tensor] | None = None) -> Tensor:
        ...
        if window_size is not None and window_size < seqlen:
            ...
            attn_mask = torch.where(
                ...
                torch.full((), -torch.finfo(q.dtype).max, device=x.device, dtype=q.dtype),
            )
            is_causal = False
...
class MLP(nn.Module):
    ...
    def forward(self, x: Tensor, fast_state: dict[str, Tensor] | None = None) -> Tensor:
        ...
        x = torch.relu(h)
        if self.training and SPEED_RT.sparse_mlp_enabled:
            x = _top2_of_4(x)
...
class Block(nn.Module):
    ...
    def forward(self, x: Tensor, x0: Tensor, fast_state: dict[str, Tensor] | None = None) -> Tensor:
        ...
        run_selective = (
            self.training
            and SPEED_RT.selective_last_n > 0
            and self.layer_idx >= max(0, SPEED_RT.total_layers - SPEED_RT.selective_last_n)
            and SPEED_RT.selective_keep < 0.999
        )
        if run_selective:
            flat = mlp_in.reshape(-1, mlp_in.size(-1))
            keep = max(1, int(round(flat.size(0) * SPEED_RT.selective_keep)))
            scores = flat.float().square().mean(dim=-1)
            idx = torch.topk(scores, k=keep, sorted=False).indices
            mlp_selected = self.mlp(flat.index_select(0, idx), fast_state=fast_state)
            mlp_out = torch.zeros_like(flat)
            mlp_out.index_copy_(0, idx, mlp_selected)
            mlp_out = mlp_out.view_as(mlp_in)
        else:
            mlp_out = self.mlp(mlp_in, fast_state=fast_state)
...
class GPT(nn.Module):
    ...
    def forward(
        ...
        active_layers = self.num_layers if SPEED_RT.active_layers is None else max(1, min(SPEED_RT.active_layers, self.num_layers))
        encoder_active = min(self.num_encoder_layers, active_layers)
        decoder_active = max(0, min(self.num_decoder_layers, active_layers - self.num_encoder_layers))

        for i in range(encoder_active):
            x = self.blocks[i](x, x0, fast_state=fast_state)
            skips.append(x)
        for i in range(decoder_active):
            if skips:
...
def main(argv: list[str] | None = None) -> None:
    ...
    if args.speed_schedule_update_every <= 0:
        raise ValueError(f"SPEED_SCHEDULE_UPDATE_EVERY must be positive, got {args.speed_schedule_update_every}")
    if args.attn_window_start < 0 or args.attn_window_final < 0:
        raise ValueError(
            f"ATTN_WINDOW_START and ATTN_WINDOW_FINAL must be non-negative, got {args.attn_window_start} and {args.attn_window_final}"
        )
    if args.attn_window_warmup_steps < 0 or args.progressive_depth_warmup_steps < 0 or args.selective_mlp_warmup_steps < 0:
        raise ValueError(
            "ATTN_WINDOW_WARMUP_STEPS, PROGRESSIVE_DEPTH_WARMUP_STEPS, and SELECTIVE_MLP_WARMUP_STEPS must be non-negative"
        )
    if not 0.0 < args.selective_mlp_keep_start <= 1.0 or not 0.0 < args.selective_mlp_keep_end <= 1.0:
        raise ValueError(
            f"SELECTIVE_MLP_KEEP_START and SELECTIVE_MLP_KEEP_END must be in (0, 1], got {args.selective_mlp_keep_start} and {args.selective_mlp_keep_end}"
        )
    ...
    use_mem_efficient_sdp = not use_flash_sdp
    use_math_sdp = not use_flash_sdp
    ...
    reset_speed_runtime(args.num_layers)
    ...
    log0(
        f"speed_features:compile_mode={args.compile_mode} sparse_mlp_24={args.sparse_mlp_24} "
        f"sparse_mlp_last_n={args.sparse_mlp_last_n} attn_window_start={args.attn_window_start} "
        f"attn_window_final={args.attn_window_final if args.attn_window_final > 0 else args.train_seq_len} "
        f"progressive_depth_start={args.progressive_depth_start} selective_mlp_tokens={args.selective_mlp_tokens} "
        f"speed_schedule_update_every={args.speed_schedule_update_every}"
    )
    ...
    if args.warmup_steps > 0:
        ...
        for warmup_step in range(args.warmup_steps):
            update_speed_runtime(args, args.num_layers, warmup_step)
            zero_grad_all()
    ...
    while True:
        ...
        if should_validate:
            speed_runtime_snapshot = snapshot_speed_runtime()
            reset_speed_runtime(args.num_layers)
            ...
            restore_speed_runtime(speed_runtime_snapshot)
            elapsed_ms = wallclock_elapsed_ms()
            log0(
        ...
        if meta_phase_active and args.use_fast_adapters:
            reset_speed_runtime(args.num_layers)
        else:
            update_speed_runtime(args, args.num_layers, step)
        meta_active = (
            meta_phase_active
